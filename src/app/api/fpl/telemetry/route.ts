...
export async function GET() {
  ...
  if (redis) {
    try {
      const [redisOdds, redisPrices, redisMomentum, redisEo] = await Promise.all([
      ...
      if (redisOdds) {
        matchOdds = typeof redisOdds === 'string' ? JSON.parse(redisOdds) : redisOdds;
        source = 'redis';
      }
      if (redisPrices) {
        priceSnapshots = typeof redisPrices === 'string' ? JSON.parse(redisPrices) : redisPrices;
        source = 'redis';
      }
      if (redisMomentum) {
        formMomentum = typeof redisMomentum === 'string' ? JSON.parse(redisMomentum) : redisMomentum;
        source = 'redis';
      }
      if (redisEo) {
        top10kOwnership = typeof redisEo === 'string' ? JSON.parse(redisEo) : redisEo;
        source = 'redis';
      }
    } catch (err) {
      console.warn('Failed to load telemetry from Redis, falling back to static bundles:', err);
    }
  }
