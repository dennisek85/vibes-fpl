        ...
        <div className="bg-slate-950/80 backdrop-blur-xl border border-white/10 rounded-3xl p-5 sm:p-7 shadow-2xl space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 pt-2">
            {backtest.driftTrajectory.map((pt) => (
              <div
                key={pt.gw}
                className="bg-slate-900/90 border border-white/10 rounded-2xl p-3.5 flex items-center justify-between shadow-md"
              >
                <div>
        ...
        <div className="bg-slate-950/80 backdrop-blur-xl border border-white/10 rounded-3xl p-5 sm:p-7 shadow-2xl space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-white/10 pb-4">
            ...
            <div className="flex items-center gap-2 bg-amber-500/10 border border-amber-500/30 px-3 py-1.5 rounded-xl font-mono text-xs text-amber-300 font-black">
              Total Armband Swing:{" "}
              {backtest.totalCaptaincyDelta >= 0
                ? `+${backtest.totalCaptaincyDelta}`
                : backtest.totalCaptaincyDelta}{" "}
              pts
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
            {backtest.captaincyComparisons.map((cap) => {
              ...
              return (
                <div
                  key={cap.gw}
                  className="bg-slate-900/90 border border-white/10 rounded-2xl p-4 shadow-md space-y-3"
                >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="bg-slate-950/80 p-3 rounded-xl border border-white/5 flex items-center gap-2.5">
                      <KitIcon
                        teamCode={uTeam?.code}
                        teamShortName={uTeam?.short_name}
                    ...
                    <div className="bg-emerald-950/30 p-3 rounded-xl border border-emerald-500/30 flex items-center gap-2.5">
                      <KitIcon
                        teamCode={aTeam?.code}
                        teamShortName={aTeam?.short_name}
                        className="w-9 h-9 shrink-0"
                      />
                      <div className="min-w-0">
                        ...
                        <span className="text-xs font-black text-emerald-200 truncate block">
                          {aPlayer?.web_name || "None"}
                        </span>
            ...
            <div className="bg-slate-900/90 border border-white/10 rounded-2xl p-4 space-y-2.5 shadow-md">
              <div className="flex items-center justify-between">
                <span className="text-xs font-black text-indigo-300 flex items-center gap-1.5 uppercase">
                  ...
                  {UI_TEXT.analytics.attribution.cleanSheets}
                </span>
              ...
              <div className="w-full bg-slate-950 rounded-full h-2 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${backtest.attribution.cleanSheets.percentage}%`,
                  }}
                ></div>
              </div>
            </div>
            ...
            <div className="bg-slate-900/90 border border-white/10 rounded-2xl p-4 space-y-2.5 shadow-md">
              <div className="flex items-center justify-between">
                <span className="text-xs font-black text-rose-300 flex items-center gap-1.5 uppercase">
                  ...
                  {UI_TEXT.analytics.attribution.goals}
                </span>
                <span className="text-xs font-mono font-black text-emerald-400">
              ...
              <div className="w-full bg-slate-950 rounded-full h-2 overflow-hidden">
                <div
                  ...
                  style={{ width: `${backtest.attribution.goals.percentage}%` }}
                ></div>
              </div>
            </div>
            ...
            <div className="bg-slate-900/90 border border-white/10 rounded-2xl p-4 space-y-2.5 shadow-md">
              <div className="flex items-center justify-between">
                <span className="text-xs font-black text-blue-300 flex items-center gap-1.5 uppercase">
                  ...
                  {UI_TEXT.analytics.attribution.assists}
                </span>
                <span className="text-xs font-mono font-black text-emerald-400">
              ...
              <div className="w-full bg-slate-950 rounded-full h-2 overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${backtest.attribution.assists.percentage}%`,
                  }}
                ></div>
              </div>
            </div>
            ...
            <div className="bg-slate-900/90 border border-white/10 rounded-2xl p-4 space-y-2.5 shadow-md">
              <div className="flex items-center justify-between">
                <span className="text-xs font-black text-amber-300 flex items-center gap-1.5 uppercase">
                  ...
                  {UI_TEXT.analytics.attribution.bonus}
                </span>
                <span className="text-xs font-mono font-black text-emerald-400">
              ...
              <div className="w-full bg-slate-950 rounded-full h-2 overflow-hidden">
                <div
                  ...
                  style={{ width: `${backtest.attribution.bonus.percentage}%` }}
                ></div>
              </div>
            </div>
            ...
            <div className="bg-slate-900/90 border border-white/10 rounded-2xl p-4 space-y-2.5 shadow-md">
              <div className="flex items-center justify-between">
                <span className="text-xs font-black text-emerald-300 flex items-center gap-1.5 uppercase">
                  ...
                  {UI_TEXT.analytics.attribution.teamValue}
                </span>
                <span className="text-xs font-mono font-black text-emerald-400">
        ...
        <div className="bg-slate-950/80 backdrop-blur-xl border border-white/10 rounded-3xl p-5 sm:p-7 shadow-2xl space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-white/10 pb-4">
            ...
            <div className="flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/30 px-3 py-1.5 rounded-xl font-mono text-xs text-emerald-300 font-black">
              Total Transfer Swing:{" "}
              {backtest.totalTransferSwing >= 0
                ? `+${backtest.totalTransferSwing}`
                : backtest.totalTransferSwing}{" "}
              pts
            </div>
          </div>
          ...
          {backtest.transferSwings.length > 0 ? (
            <div className="space-y-3 pt-2">
              {backtest.transferSwings.map((t, idx) => (
                <div
                  key={idx}
                  className="bg-slate-900/90 border border-white/10 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-md"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-xs font-medium text-slate-300">
                      ...
                      <strong className="text-white">
                        {t.userTransfer?.playerOut.web_name}
                      </strong>{" "}
                      ➔{" "}
                      <strong className="text-emerald-400">
                        {t.userTransfer?.playerIn.web_name}
                      </strong>{" "}
                      ({t.userTransfer?.netPoints} pts)
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between sm:justify-end gap-4 border-t sm:border-t-0 border-white/5 pt-2 sm:pt-0">
                    <div className="text-xs font-medium text-slate-300">
                      AI #1:{" "}
                      <strong className="text-white">
                        {t.aiTransfer?.playerOut.web_name}
                      </strong>{" "}
                      ➔{" "}
                      <strong className="text-cyan-400">
                        {t.aiTransfer?.playerIn.web_name}
                      </strong>{" "}
                      ({t.aiTransfer?.netPoints} pts)
                    </div>
                    <span
            ...
            <div className="py-10 text-center text-slate-400 text-xs sm:text-sm font-medium">
              No historical transfers recorded yet. Transfer swing decisions
              will appear here as gameweeks complete!
            </div>
          )}
        </div>
      )}
