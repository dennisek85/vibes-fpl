...
import {
  solveOptimalSquad,
  OptimalSquadResult,
} from "@/utils/aiOptimalSquadSolver";
...
export const AiScoutModal: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    "transfers" | "targets" | "hits" | "chips" | "optimal_squad"
  >("transfers");
  ...
  const [appliedTransferMsg, setAppliedTransferMsg] = useState<string | null>(
    null,
  );
  ...
  const transfersInIds = useMemo(
    () => currentPlan?.transfersIn || [],
    [currentPlan?.transfersIn],
  );
  const transfersOutIds = useMemo(
    () => currentPlan?.transfersOut || [],
    [currentPlan?.transfersOut],
  );
  ...
  const chipRadar = useMemo(() => {
    if (!isScoutModalOpen) return [];
    return getChipRadarRecommendations(
      squad,
      players,
      selectedGameweek,
      getPlayerGameweekXp,
  ...
  const handleApplyTransfer = (rec: TransferRecommendation) => {
    ...
    setAppliedTransferMsg(
      `Applied: ${rec.playerIn.web_name} in for ${rec.playerOut.web_name}!`,
    );
  ...
  const handleApplyDoubleTransfer = (combo: DoubleTransferHitCombo) => {
    ...
    setTimeout(() => {
      ...
      setAppliedTransferMsg(
        `Applied Double Move (-4 hit): +${combo.netProfitAfterHit} net pts profit!`,
      );
      setTimeout(() => setAppliedTransferMsg(null), 3500);
    }, 100);
  };
  ...
  const handleApplyOptimalSquad = () => {
    ...
    setAppliedTransferMsg(
      `✨ Applied Strongest 15-Man Squad (${optimalSquad.formation}) · ${optimalSquad.captain.web_name} (C) · ${optimalSquad.totalProjectedPoints} xP!`,
    );
    ...
    >
      ...
      >
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg sm:text-xl font-black text-white">
                  AI Transfer Scout &amp; Market Intelligence
                </h2>
              ...
              <p className="text-xs text-slate-400">
                Machine learning forecasts for Gameweek {selectedGameweek} ·
                Bank:{" "}
                <strong className="text-emerald-400">
                  {formatMoney(currentPlan?.calculatedBank || 0, true)}
                </strong>
              </p>
            </div>
          </div>
        ...
        <div className="p-4 sm:p-6 overflow-y-auto flex-1 space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between flex-wrap gap-2 pb-2">
                <span className="text-xs text-slate-400 font-medium">
                  Evaluating all affordable, legal moves for your exact 15-man
                  squad:
                </span>
              ...
              {recommendations.length === 0 ? (
                <div className="text-center py-12 bg-slate-950/50 rounded-3xl border border-white/5 p-6">
                  ...
                  <p className="text-xs text-slate-400 mt-1">
                    Your current squad is already highly optimal for this
                    gameweek within your bank budget!
                  </p>
                </div>
              ) : (
                        ...
                        <div className="flex items-center gap-3 sm:gap-4 w-full sm:w-auto">
                          ...
                          <div className="flex flex-col text-left">
                            ...
                            <span className="text-[11px] text-emerald-400/90 font-mono font-bold">
                              {inTeam?.short_name} · £
                              {(rec.playerIn.now_cost / 10).toFixed(1)}m
                            </span>
                          </div>
                        </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {(
                  (targetPosition === "value"
                    ? dreamTargets.topValuePicks
                    : dreamTargets.topByPosition[targetPosition as number]) ||
                  []
                ).map((target, idx) => (
                  <div
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col text-left">
                        ...
                        <span className="text-[11px] text-slate-400 font-mono">
                          ...
                          {(target.player.now_cost / 10).toFixed(1)}m
                        </span>
                      </div>
                    </div>
          ...
          {activeTab === "hits" && hitAnalysis && (
            <div className="space-y-5">
              ...
              <div
                className={`p-4 sm:p-5 rounded-3xl border shadow-xl flex flex-col gap-2.5 ${
                ...
                {hitAnalysis.hasPlannedHit && (
                  <div className="grid grid-cols-3 gap-2 pt-2 border-t border-white/10 text-center">
                    <div className="p-2 bg-slate-900/80 rounded-xl">
                      ...
                      <span
                        className={`text-xs sm:text-sm font-black font-mono ${hitAnalysis.netImmediateGain >= 0 ? "text-emerald-400" : "text-rose-400"}`}
                      >
                        {hitAnalysis.netImmediateGain >= 0
                          ? `+${hitAnalysis.netImmediateGain}`
                          : hitAnalysis.netImmediateGain}{" "}
                        pts
                      </span>
                    </div>
                    <div className="p-2 bg-slate-900/80 rounded-xl">
                      ...
                      <span
                        className={`text-xs sm:text-sm font-black font-mono ${hitAnalysis.netTwoGwGain >= 0 ? "text-emerald-400" : "text-rose-400"}`}
                      >
                        {hitAnalysis.netTwoGwGain >= 0
                          ? `+${hitAnalysis.netTwoGwGain}`
                          : hitAnalysis.netTwoGwGain}{" "}
                        pts
                      </span>
                    </div>
                    <div className="p-2 bg-slate-900/80 rounded-xl">
                      ...
                      <span
                        className={`text-xs sm:text-sm font-black font-mono ${hitAnalysis.netThreeGwGain >= 0 ? "text-emerald-400" : "text-rose-400"}`}
                      >
                        {hitAnalysis.netThreeGwGain >= 0
                          ? `+${hitAnalysis.netThreeGwGain}`
                          : hitAnalysis.netThreeGwGain}{" "}
                        pts
                      </span>
                    </div>
              ...
              <div className="space-y-3">
                ...
                {hitAnalysis.bestDoubleTransferCombos.length === 0 ? (
                  <div className="text-center py-6 bg-slate-950/40 rounded-2xl border border-white/5 p-4">
                    <p className="text-xs text-slate-400">
                      No 2-player swap combos generate enough point divergence
                      to justify a -4 penalty. Rolling your transfer is
          ...
          {activeTab === "chips" && (
            <div className="space-y-4">
              <span className="text-xs text-slate-400 font-medium block">
                OpenFPL model forecast radar detecting optimal Gameweeks for
                chip deployment:
              </span>
          ...
          {activeTab === "optimal_squad" && (
            <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-300">
                      ...
                      <strong className="text-emerald-400 font-mono">
                        {formatMoney(totalAvailableBudget, true)}
                      </strong>
                      {optimalSquad && (
                        <span>
                          {" "}
                          ...
                          <strong className="text-white font-mono">
                            {formatMoney(optimalSquad.totalCost, true)}
                          </strong>{" "}
                          ({formatMoney(optimalSquad.remainingBank, true)} ITB)
                        </span>
                      )}
                    </p>
                  </div>
              ...
              {optimalSquad &&
                (() => {
                  const gkStarters = optimalSquad.starters.filter(
                    (s) => s.player.element_type === 1,
                  );
                  const defStarters = optimalSquad.starters.filter(
                    (s) => s.player.element_type === 2,
                  );
                  const midStarters = optimalSquad.starters.filter(
                    (s) => s.player.element_type === 3,
                  );
                  const fwdStarters = optimalSquad.starters.filter(
                    (s) => s.player.element_type === 4,
                  );
                    ...
                    return (
                      ...
                      >
                        ...
                        {(isCaptain || isViceCaptain) && (
                          <div
                            className={`absolute -top-1 -left-1 sm:top-0 sm:left-1 w-5 h-5 rounded-full flex items-center justify-center font-black text-[10px] shadow-lg border z-20 ${
                              isCaptain
                        ...
                        <div className="w-full mt-1 flex flex-col items-center bg-slate-950/95 backdrop-blur-md rounded-xl border border-white/20 shadow-xl overflow-hidden text-center">
                          ...
                          <div className="w-full py-0.5 sm:py-1 px-1 bg-emerald-950/90 text-emerald-300 font-mono font-black text-[10px] sm:text-xs text-center flex items-center justify-center">
                            <span>
                              {xp.toFixed(1)} xP{horizon !== "1gw" ? "/GW" : ""}
                            </span>
                          </div>
                        </div>
                      </div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-white font-bold font-sans">
                              ...
                              <strong className="text-amber-300">
                                {optimalSquad.captain.web_name} (C)
                              </strong>{" "}
                              ·{" "}
                              <strong className="text-slate-300">
                                {optimalSquad.viceCaptain.web_name} (V)
                              </strong>
                            </span>
                          </div>
                          <div className="flex items-center gap-4">
                            <span>
                              ...
                              <strong className="text-emerald-400 font-black text-xs sm:text-sm">
                                {optimalSquad.totalProjectedPoints} pts/GW
                              </strong>
                            </span>
                            <span>
                              Gain:{" "}
                              <strong className="text-cyan-300 font-black text-xs sm:text-sm">
                                +
                                {optimalSquad.xpGain > 0
                                  ? optimalSquad.xpGain
                                  : 0}{" "}
                                /GW
                              </strong>
                            ...
                            <span>
                              ...
                              <strong className="text-amber-400 font-bold">
                                {optimalSquad.transfersCount} / 15
                              </strong>
                            </span>
                          </div>
                        </div>
                            ...
                            return (
                              ...
                              >
                                <div className="relative shrink-0">
                                  <span className="absolute -top-1 -left-1 text-[8px] sm:text-[9px] bg-slate-800 text-slate-300 px-1.5 py-0.2 rounded font-bold border border-white/10 uppercase">
                                    {label}
                                  </span>
                                  <KitIcon
                                    teamCode={team?.code}
                        ...
                        <div className="pt-2 border-t border-white/10 text-center text-[10px] sm:text-xs text-slate-400 font-mono">
                          <span>
                            ...
                            <strong className="text-slate-200">
                              {formatMoney(
                                optimalSquad.bench.reduce(
                                  (s, b) => s + b.player.now_cost,
                                  0,
                                ),
                                true,
                              )}
        ...
        <div className="p-3 sm:p-4 border-t border-white/10 bg-slate-950/70 flex items-center justify-between text-xs text-slate-400">
          <span>
            AI predictions updated automatically before every deadline
          </span>
          <button
            onClick={closeScoutModal}
