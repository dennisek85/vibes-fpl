...
export const StrategyBar: React.FC<StrategyBarProps> = ({ onOpenOverrides }) => {
  const { 
    selectedGameweek, 
    gameweekPlans, 
  ...
  const isBankOverridden = plan.bankOverride !== null && plan.bankOverride !== undefined;
  const isFTOverridden = plan.freeTransfersOverride !== null && plan.freeTransfersOverride !== undefined;
    ...
    <div className="w-full max-w-5xl mx-auto my-2.5 p-3 sm:p-4 bg-slate-900/95 backdrop-blur-md rounded-2xl sm:rounded-3xl border border-white/15 shadow-2xl flex flex-wrap items-center justify-between gap-3 text-slate-200">
      ...
      <div className="flex items-center gap-3 sm:gap-5 flex-wrap">
        ...
        >
          ...
          <div className="flex flex-col">
            <span className="text-[11px] font-semibold text-slate-400 leading-none">Projected Bank</span>
            <span className="text-sm sm:text-base font-black text-emerald-400 font-mono leading-tight flex items-center gap-1.5 mt-0.5">
              ...
              {isBankOverridden && <span className="text-[10px] text-amber-400 font-sans font-normal">(override)</span>}
            </span>
          </div>
          <Edit3 className="w-3.5 h-3.5 text-slate-500 opacity-0 group-hover:opacity-100 transition-opacity ml-1" />
        </div>
