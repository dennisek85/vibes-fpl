...
import { X, Calendar, History, ShoppingBag, Loader2 } from "lucide-react";
...
export const PlayerDetailModal: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<"season" | "past" | "fixtures">(
    "season",
  );
  const [imgError, setImgError] = useState(false);

  const player = selectedPlayerForDetail
    ? playerMap.get(selectedPlayerForDetail)
    : null;
  ...
  const cleanPhotoCode = player.photo
    ? player.photo.replace(".jpg", ".png").replace(/^p/, "")
    : null;
      ...
      >
              ...
              <div className="flex flex-wrap items-center gap-2 mt-1.5">
                ...
                {(() => {
                  ...
                  return sp.roles.map((role, idx) => (
                    <span
                      key={idx}
                      className="text-[11px] sm:text-xs font-mono font-black text-amber-300 bg-amber-950/80 px-2.5 py-0.5 rounded-full border border-amber-500/40 shadow flex items-center gap-1"
                    >
                      {role.includes("PEN")
                        ? "🎯 "
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 divide-x divide-white/10 border-b border-white/10 bg-slate-950/90 text-center py-4 sm:py-5">
          ...
          <div className="px-2">
            ...
            <span className="text-lg sm:text-2xl md:text-3xl font-black text-white font-mono mt-0.5 block">
              {player.total_points && player.starts
                ? (player.total_points / Math.max(1, player.starts)).toFixed(1)
                : player.form || "0.0"}
            </span>
          </div>
          <div className="px-2">
            ...
            <span className="text-lg sm:text-2xl md:text-3xl font-black text-emerald-400 font-mono mt-0.5 block">
          ...
          <div className="px-2">
            ...
            <span className="text-lg sm:text-2xl md:text-3xl font-black text-amber-400 font-mono mt-0.5 block">
              {player.bps || 0}
            </span>
          </div>
          <div className="px-2">
            ...
            <span className="text-lg sm:text-2xl md:text-3xl font-black text-teal-300 font-mono mt-0.5 block">
              {player.threat
                ? (parseFloat(player.threat) / 10).toFixed(1)
                : "0.0"}
            </span>
          </div>
          <div className="px-2">
            ...
            <span className="text-lg sm:text-2xl md:text-3xl font-black text-purple-300 font-mono mt-0.5 block">
              ...
              {eo && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[10px] text-slate-400 font-mono">
                    ({eo.ownership}% own · {eo.captaincy}% cap)
                  </span>
                  <span
        ...
        {(() => {
          ...
          return (
            <div
              className={`px-5 sm:px-6 py-3.5 border-b border-white/10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 ${
                risk.riskLevel === "high"
                  ? "bg-rose-950/70 border-rose-500/30"
                  : risk.riskLevel === "medium" || risk.riskLevel === "doubtful"
                    ? "bg-amber-950/70 border-amber-500/30"
                    : risk.riskLevel === "fatigue"
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span
                      className={`px-2.5 py-0.5 rounded-full font-black text-xs font-mono border ${
                        risk.riskLevel === "high"
                    ...
                    >
                      {UI_TEXT.rotationRisk.badges.startProbability(
                        risk.startProbability,
                      )}
                    </span>
                  </div>
              ...
              {summaryData?.history && summaryData.history.length > 0 ? (
                summaryData.history.slice(-5).map((m) => {
                  ...
                  return (
                    ...
                    >
                      <span className="text-[11px] sm:text-xs font-extrabold text-slate-400 uppercase tracking-wide">
                        GW{m.round}
                      </span>
                ...
                const diffColor =
                  diff === 1 || diff === 2
                    ...
                    : diff === 3
                      ...
                      : diff === 4
                        ? "bg-[#e90052] text-white font-black"
                        : "bg-[#6a041d] text-rose-100 font-black";
                const xpVal =
                  f.xP !== undefined
                    ? f.xP
                    : getPlayerGameweekXp(player.id, f.event);
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-col items-center gap-1 mt-1.5 w-full">
                      <span
                        className={`text-[10px] sm:text-xs font-mono font-black px-1.5 py-0.5 rounded-lg w-full shadow-sm truncate ${diffColor}`}
                      >
                        FDR {diff}
                      </span>
                      {showAiPredictions && (
          ...
          {loading ? (
              ...
              <table className="w-full text-left font-mono text-xs sm:text-sm border-collapse min-w-[900px]">
                <thead>
                  <tr className="bg-slate-950 border-b border-white/15 text-[11px] uppercase font-black tracking-wider text-slate-400">
                    ...
                    <th className="py-3 px-2 text-center font-bold text-emerald-400">
                      Pts
                    </th>
                  ...
                  {summaryData?.history && summaryData.history.length > 0 ? (
                    summaryData.history.map((m) => {
                      ...
                      const resultStr =
                        m.team_h_score !== null &&
                        m.team_a_score !== null &&
                        m.team_h_score !== undefined &&
                        m.team_a_score !== undefined
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-2.5 px-2 text-center font-black text-emerald-400 bg-emerald-950/20">
                            {m.total_points}
                          </td>
                          ...
                          <td className="py-2.5 px-2 text-center text-rose-300">
                            {parseFloat(m.expected_goals || "0").toFixed(2)}
                          </td>
                          <td className="py-2.5 px-2 text-center text-blue-300">
                            {parseFloat(m.expected_assists || "0").toFixed(2)}
                          </td>
                          <td className="py-2.5 px-2 text-center text-amber-300">
                            {parseFloat(
                              m.expected_goal_involvements || "0",
                            ).toFixed(2)}
                          </td>
                    ...
                    <tr>
                      <td
                        colSpan={15}
                        className="py-8 text-center text-slate-500 font-sans text-sm"
                      >
                ...
                {seasonTotals && (
                  <tfoot className="bg-slate-950 font-black border-t-2 border-white/20 text-white">
                    <tr>
                      <td
                        className="py-3 px-3.5 uppercase text-emerald-400"
                        colSpan={3}
                      >
                        Totals
                      </td>
                      <td className="py-3 px-2 text-center text-emerald-400 bg-emerald-950/40">
                        {seasonTotals.totals.points}
                      </td>
                      <td className="py-3 px-2 text-center">
                        {seasonTotals.totals.minutes}
                      </td>
                      <td className="py-3 px-2 text-center">
                        {seasonTotals.totals.goals}
                      </td>
                      <td className="py-3 px-2 text-center">
                        {seasonTotals.totals.assists}
                      </td>
                      <td className="py-3 px-2 text-center text-rose-300">
                        {seasonTotals.totals.xG.toFixed(2)}
                      </td>
                      <td className="py-3 px-2 text-center text-blue-300">
                        {seasonTotals.totals.xA.toFixed(2)}
                      </td>
                      <td className="py-3 px-2 text-center text-amber-300">
                        {seasonTotals.totals.xGI.toFixed(2)}
                      </td>
                      <td className="py-3 px-2 text-center text-indigo-300">
                        {seasonTotals.totals.clean_sheets}
                      </td>
                      <td className="py-3 px-2 text-center">
                        {seasonTotals.totals.goals_conceded}
                      </td>
                      <td className="py-3 px-2 text-center">
                        {seasonTotals.totals.xGC.toFixed(2)}
                      </td>
                      <td className="py-3 px-2 text-center">
                        {seasonTotals.totals.bps}
                      </td>
                      <td className="py-3 px-2 text-center text-amber-400">
                        {seasonTotals.totals.bonus}
                      </td>
                    </tr>
                    <tr className="text-slate-400 text-xs bg-slate-900/60">
                      <td className="py-2.5 px-3.5 uppercase" colSpan={3}>
                        Per 90 Mins
                      </td>
                      <td className="py-2.5 px-2 text-center text-slate-400">
                        -
                      ...
                      <td className="py-2.5 px-2 text-center">-</td>
                      <td className="py-2.5 px-2 text-center text-rose-300">
                        {seasonTotals.per90.xG}
                      </td>
                      <td className="py-2.5 px-2 text-center text-blue-300">
                        {seasonTotals.per90.xA}
                      </td>
                      <td className="py-2.5 px-2 text-center text-amber-300">
                        {seasonTotals.per90.xGI}
                      </td>
                      <td className="py-2.5 px-2 text-center">-</td>
                      <td className="py-2.5 px-2 text-center">-</td>
                      <td className="py-2.5 px-2 text-center">
                        {seasonTotals.per90.xGC}
                      </td>
                      <td className="py-2.5 px-2 text-center">-</td>
                      <td className="py-2.5 px-2 text-center">-</td>
                    </tr>
            ...
            <div className="w-full overflow-x-auto">
              <table className="w-full text-left font-mono text-xs sm:text-sm border-collapse">
                ...
                <tbody className="divide-y divide-white/5 text-slate-300">
                  {summaryData?.history_past &&
                  summaryData.history_past.length > 0 ? (
                    summaryData.history_past.map((p) => (
                      ...
                      >
                        ...
                        <td className="py-2.5 px-3.5 text-center font-black text-emerald-400 bg-emerald-950/20">
                          {p.total_points}
                        </td>
                        ...
                        <td className="py-2.5 px-3.5 text-center text-indigo-300">
                          {p.clean_sheets}
                        </td>
                        <td className="py-2.5 px-3.5 text-center text-amber-400">
                          {p.bonus}
                        </td>
                        <td className="py-2.5 px-3.5 text-center text-slate-400">
                          £{(p.start_cost / 10).toFixed(1)}m
                        </td>
                        <td className="py-2.5 px-3.5 text-center text-emerald-400 font-bold">
                          £{(p.end_cost / 10).toFixed(1)}m
                        </td>
                      </tr>
                    ))
                  ) : (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3.5">
              {summaryData?.fixtures?.map((f) => {
                ...
                return (
                  <div
                    key={f.id || f.event}
                    className="p-4 bg-slate-950 rounded-2xl border border-white/10 flex flex-col justify-between shadow-md"
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-black text-white">
                        {f.event_name || `GW ${f.event}`}
                      </span>
                      <span
                        className={`text-[11px] font-bold px-2.5 py-0.5 rounded-full ${diffColor}`}
                      >
                        FDR {diff}
                      </span>
                    </div>
