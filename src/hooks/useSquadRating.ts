...
export function useSquadRating(): SquadRatingResult | null {
  ...
  const activePlan = usePlannerStore(state => state.gameweekPlans[selectedGameweek]);
  ...
  return useMemo(() => {
    if (!showAiPredictions || !activePlan?.squad || activePlan.squad.length === 0 || !players.length) {
      return null;
    }
