...
export function useSquadTelemetry(): SquadTelemetryResult {
  ...
  const getPlayerGameweekActualPoints = usePlannerStore(
    (state) => state.getPlayerGameweekActualPoints,
  );
  const isGameweekLocked = usePlannerStore((state) => state.isGameweekLocked);
  const teamHistoryCurrent = usePlannerStore(
    (state) => state.teamHistoryCurrent,
  );
  ...
  if (currentPlan?.squad) {
    ...
    currentPlan.squad.forEach((pick) => {
      ...
      const rawPoints = getPlayerGameweekActualPoints(
        pick.element,
        selectedGameweek,
      );
      if (rawPoints !== null) {
        hasLivePlayerPoints = true;
        const mult = isStarting
          ? pick.is_captain
            ? isTripleCaptain
              ? 3
              : 2
  ...
  const gwHistory = teamHistoryCurrent.find(
    (h) => h.event === selectedGameweek,
  );
  ...
  const gameweekActualPoints = hasLivePlayerPoints
    ...
    : historyPoints !== null
      ? historyPoints
      : isLocked &&
          selectedGameweek === teamSummary?.current_event &&
