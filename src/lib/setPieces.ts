...
export function getPlayerSetPieceProfile(
  ...
  for (const teamSet of relevantTeams) {
    ...
    if (teamSet.penalties && teamSet.penalties.length > 0) {
      if (teamSet.penalties[0].includes(webName) || normalized.includes(teamSet.penalties[0])) {
        result.isPrimaryPenalty = true;
        result.roles.push('PEN 1st');
        result.addedXg += 0.16;
      } else if (teamSet.penalties.slice(1).some(p => p.includes(webName) || normalized.includes(p))) {
    ...
    if (teamSet.corners && teamSet.corners.length > 0) {
      if (teamSet.corners.slice(0, 2).some(c => c.includes(webName) || normalized.includes(c))) {
    ...
    if (teamSet.directFreeKicks && teamSet.directFreeKicks.length > 0) {
      if (teamSet.directFreeKicks.slice(0, 2).some(f => f.includes(webName) || normalized.includes(f))) {
    ...
    if (teamSet.indirectFreeKicks && teamSet.indirectFreeKicks.length > 0) {
      if (teamSet.indirectFreeKicks.slice(0, 2).some(f => f.includes(webName) || normalized.includes(f))) {
