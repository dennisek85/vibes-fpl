...
export function getPlayerSetPieceProfile(
  ...
  for (const teamSet of relevantTeams) {
    // 1. Penalties (OpenFPL Spec: +0.16 xG for 1st taker)
    if (teamSet.penalties && teamSet.penalties.length > 0) {
      if (matchesTaker(teamSet.penalties[0], webName, secondName, firstName)) {
        ...
        result.addedXg += 0.16; // Primary penalty taker
      } else if (
        teamSet.penalties
          .slice(1)
    ...
    // 2. Corners (OpenFPL Spec: +0.14 xA for primary taker)
    if (teamSet.corners && teamSet.corners.length > 0) {
      if (matchesTaker(teamSet.corners[0], webName, secondName, firstName)) {
        ...
        result.addedXa += 0.14; // Primary corner crosser (~65% volume share)
      } else if (
        ...
        result.addedXa += 0.05; // Secondary corner crosser (~30% volume share)
      }
    }

    // 3. Direct Free-Kicks (OpenFPL Spec: +0.06 xG for primary taker)
