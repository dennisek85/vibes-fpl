...
export function calculatePlayerPricePrediction(
  ...
  const isOfficialLocked = player.price_change_locked_until !== null && player.price_change_locked_until !== undefined;
  const isLocked = isOfficialLocked || player.status === 'u' || isCostChangeLocked || hasMaxWeeklyChanges;
  ...
  if (player.price_change_percent !== undefined && player.price_change_percent !== null) {
    const parsedPercent = typeof player.price_change_percent === 'string' 
    ...
    const rawHourly = player.price_change_hourly_rate !== undefined ? player.price_change_hourly_rate : 0;
    if (typeof rawHourly === 'number' && rawHourly !== 0) {
      if (Math.abs(rawHourly) > 50) {
        ...
        const impliedThreshold = targetProgress !== 0 ? Math.abs(rawNetTransfers / (targetProgress / 100)) : 50000;
        hourlyVelocity = Math.round((rawHourly / impliedThreshold) * 10000) / 100;
      } else {
        hourlyVelocity = Math.round(rawHourly * 100) / 100;
      }
    } else {
      hourlyVelocity = targetProgress > 0 ? 1.50 : targetProgress < 0 ? -1.00 : 0.00;
    }

    hourlyVelocityText = hourlyVelocity > 0 ? `+${hourlyVelocity.toFixed(2)}%/hr` : hourlyVelocity < 0 ? `${hourlyVelocity.toFixed(2)}%/hr` : '0.00%/hr';
    ...
    if (isLocked) {
      ...
      targetProgress = Math.round(((effectiveNet / riseThreshold) * 100) * 10) / 10;
    } else if (rawNetTransfers < 0) {
      ...
      if (ownershipPercent <= 0.3) {
        ...
        baseFallThreshold = Math.max(4500, startingOwners * (isDefenderOrGk ? 0.030 : 0.0265));
      ...
      targetProgress = -Math.round(((Math.abs(effectiveNet) / fallThreshold) * 100) * 10) / 10;
    }
