...
export function getSmartTransferRecommendations(
  ...
  const squadPlayers = squad.map(p => playerMap.get(p.element)).filter(Boolean) as FPLPlayer[];
  ...
  for (const pOut of squadPlayers) {
    ...
    const candidates = players.filter(pIn => {
      ...
      const netTeamCount = (pIn.team === outTeamId) ? currentTeamCount : currentTeamCount + 1;
  ...
  return recommendations.sort((a, b) => (b.xpGain3Gw + b.xpGainImmediate * 1.5) - (a.xpGain3Gw + a.xpGainImmediate * 1.5));
}
...
export function getDreamTargets(
  ...
  const activePlayers = players.filter(p => p.status !== 'u' && p.status !== 'i');
  ...
  const topByPosition: Record<number, DreamTargetPick[]> = {
    1: enriched.filter(p => p.player.element_type === 1).sort((a, b) => b.xp3Gw - a.xp3Gw).slice(0, 8),
    2: enriched.filter(p => p.player.element_type === 2).sort((a, b) => b.xp3Gw - a.xp3Gw).slice(0, 10),
    3: enriched.filter(p => p.player.element_type === 3).sort((a, b) => b.xp3Gw - a.xp3Gw).slice(0, 10),
    4: enriched.filter(p => p.player.element_type === 4).sort((a, b) => b.xp3Gw - a.xp3Gw).slice(0, 8),
  };
...
export function getChipRadarRecommendations(
  ...
  for (let gw = selectedGameweek; gw <= Math.min(38, selectedGameweek + 8); gw++) {
  ...
  for (let gw = selectedGameweek; gw <= Math.min(38, selectedGameweek + 8); gw++) {
    ...
    const benchXp = benchPicks.reduce((sum, p) => sum + getXp(p.element, gw), 0);
    if (benchXp > bestBbXp) {
      bestBbXp = benchXp;
      bestBbGw = gw;
    }
  ...
  radar.push({
    ...
    subtext: 'Major Premier League fixture swing horizon with high expected returns on restructure',
...
export function analyzeTransferHit(
  ...
  const netImmediateGain = Math.round((immediateGain - hitPenaltyPoints) * 10) / 10;
  ...
  if (squadPlayers.length === 15 && availableFT === 1) {
    // Check top pairs
    const midFwds = squadPlayers.filter(p => p.element_type === 3 || p.element_type === 4);
    for (let i = 0; i < midFwds.length; i++) {
      for (let j = i + 1; j < midFwds.length; j++) {
        const out1 = midFwds[i];
        const out2 = midFwds[j];
        const totalBudget = (out1.now_cost + out2.now_cost) / 10.0 + currentBank;

        const outTwoGw = (
          getXp(out1.id, selectedGameweek) + getXp(out1.id, selectedGameweek + 1) +
          getXp(out2.id, selectedGameweek) + getXp(out2.id, selectedGameweek + 1)
        );

        // Find candidate replacements
        const cands1 = players.filter(p => p.element_type === out1.element_type && p.id !== out1.id && p.id !== out2.id && p.status !== 'i' && p.status !== 'u');
        const cands2 = players.filter(p => p.element_type === out2.element_type && p.id !== out1.id && p.id !== out2.id && p.status !== 'i' && p.status !== 'u');

        // Sample top candidates
        const topCands1 = cands1.sort((a, b) => getXp(b.id, selectedGameweek) - getXp(a.id, selectedGameweek)).slice(0, 4);
        const topCands2 = cands2.sort((a, b) => getXp(b.id, selectedGameweek) - getXp(a.id, selectedGameweek)).slice(0, 4);
        ...
        for (const in1 of topCands1) {
          for (const in2 of topCands2) {
            ...
            const inTwoGw = (
              getXp(in1.id, selectedGameweek) + getXp(in1.id, selectedGameweek + 1) +
              getXp(in2.id, selectedGameweek) + getXp(in2.id, selectedGameweek + 1)
            );

            const twoGwPointDelta = Math.round((inTwoGw - outTwoGw) * 10) / 10;
            const netProfitAfterHit = Math.round((twoGwPointDelta - 4.0) * 10) / 10;

            if (netProfitAfterHit >= 2.5) {
              const costDiff = Math.round((combinedCost - (out1.now_cost + out2.now_cost) / 10.0) * 10) / 10;
  ...
  bestDoubleTransferCombos.sort((a, b) => b.netProfitAfterHit - a.netProfitAfterHit);
